...
The **Hub-Spoke Networking** step (Step 11) deploys your AKS cluster into an enterprise hub-spoke topology, which is the recommended pattern for Azure enterprise landing zones.
...
| **On-premises connectivity** | A VPN Gateway in the hub (with `GatewaySubnet`) enables site-to-site VPN or ExpressRoute to on-premises networks. With gateway transit enabled on VNet peerings, all spoke VNets reach on-premises without their own gateways. |
...
|----------|-------------|
| **🆕 Create New Hub** | You do not have an existing hub VNet. The wizard generates hub VNet, spoke VNet, peering, and optional Azure Firewall/Bastion/VPN Gateway resources. |
...
When creating a new hub, the following optional services can be provisioned:

| Service | Subnet | Description |
|---------|--------|-------------|
| **Azure Firewall** | `AzureFirewallSubnet` (/26) | Centralised egress filtering with FQDN rules. Enable "Route AKS Egress Through Firewall" to add a UDR on the AKS subnet. |
| **Azure Bastion** | `AzureBastionSubnet` (/26) | Browser-based SSH/RDP without public IPs. Recommended with Private AKS Cluster. |
| **VPN Gateway** | `GatewaySubnet` (/27) | Zone-redundant VPN Gateway (VpnGw2AZ) for site-to-site VPN or ExpressRoute. Enables gateway transit so spoke VNets reach on-premises without their own gateways. |
...
- Configures a **private API server endpoint** if **Private AKS Cluster** is enabled
...
5. Under **Hub Services**, configure optional components:
   ...
   - Enable **VPN Gateway** to provision a zone-redundant `VpnGw2AZ` gateway and `GatewaySubnet` in the hub for site-to-site VPN or ExpressRoute connectivity.
   - Enable **Private AKS Cluster** to remove the public Kubernetes API endpoint.
6. Proceed to **Templates** — the wizard appends spoke VNet, hub VNet (if new), peering, UDR, Firewall, Bastion, and VPN Gateway resources to the generated Bicep and Terraform templates. AKS nodes are automatically placed in the spoke subnet.
